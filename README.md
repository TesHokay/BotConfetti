...
- Админы могут выгружать таблицу заявок прямо из панели: бот присылает XLSX-файл
  с кликабельной ссылкой на подтверждение оплаты в отдельной колонке.
...
2. **Запустите экспорт.** Бот соберёт все заявки, добавит комментарии по оплате и сформирует кликабельную ссылку на подтверждение оплаты в отдельной колонке. Если фото не прикреплено, в таблице появится текст «Оплата ожидается».
